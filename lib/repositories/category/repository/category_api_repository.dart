...
class CategoryApiRepository implements CategoryRepository {
  CategoryController categoryController = CategoryController.category;

  @override
  Future<Response> categoryRootHandler(Request req) async {
        ...
        case BaseRepository.category:
          if (req.method == RequestType.GET.name || req.method == RequestType.PUT.name) {
            ...
            if (uuid == null) {
              ...
              response = await categoryController.getCategoryFromUuidResponse(uuid);
            }
          } else {
            response = await categoryController.addCategory((await req.readAsString()).convertJsonCamelToSnake);
          }
          break;
        case BaseRepository.categoryDelete:
          if (req.method == RequestType.DELETE.name) {
            ...
            if (uuid == null) {
              ...
              response = await categoryController.deleteCategoryFromUuidResponse(uuid);
            }
          } else {
            response = Response(200);
          }
          break;
        case BaseRepository.categoryStatus:
          String? uuid = queryParam['uuid'];
          ...
          if (uuid == null || active == null) {
            ...
            response = await categoryController.deactivateCategoryFromUuidResponse(uuid, active);
          }
          break;
        case BaseRepository.categoryList:
          String requestBody = (await req.readAsString()).convertJsonCamelToSnake;
          response = await categoryController.getCategoryListResponse(jsonDecode(requestBody));
          break;
        default:
